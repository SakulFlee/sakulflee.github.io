...
export default class TimeLine extends React.Component {
    ...
    point = styled.div`
        width: 16px;
        height: 16px;
        border-right: solid blue 8px 16px;
    `;

    timelinePoint(props: {color: string, text:string, tooltip: string}): JSX.Element {
        let textColor = (props.color === 'warning' || props.color === 'white') ? 'has-text-black' : 'has-text-white';
        return <div className={`tag is-large is-${props.color} ${textColor} tooltip is-tooltip-${props.color}`} data-tooltip={props.tooltip} style={{width: "90%"}}>{props.text}</div>;
    }
